...
class ControlConnection extends events.EventEmitter {
  ...
  async _refreshHosts(initializing, isReconnecting) {
    ...
    if (isReconnecting && this.options.isMetadataSyncEnabled) {
      this.log('info',
        `Refreshing all keyspaces metadata after reconnecting using ${c.endpointFriendlyName}`);
      await this.metadata.refreshKeyspacesInternal(false);
      this.log('info',
        `Finished refreshing all keyspaces metadata after reconnecting using ${c.endpointFriendlyName}`);
    }
  ...
  async _refresh(hostIterator) {
    if (this._refreshPromise) {
      this.log('info', 'ControlConnection refresh requested while one is already in progress, reusing it');
      return await this._refreshPromise;
    }

    this.log('info', 'Starting ControlConnection refresh');
    ...
    try {
      ...
      this._refreshPromise = null;
      this.log('info', 'Finished ControlConnection refresh');
    }
  }
    ...
    if (event.isKeyspace) {
      if (event.schemaChangeType === schemaChangeTypes.dropped) {
        handler = function removeKeyspace() {
          ...
          self.log('info', `Removing dropped keyspace ${event.keyspace} from metadata`);
