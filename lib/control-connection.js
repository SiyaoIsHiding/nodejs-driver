...
// Process-wide counter used to give each ControlConnection a stable id for log correlation
let controlConnectionCounter = 0;
...
class ControlConnection extends events.EventEmitter {
  ...
  constructor(options, profileManager, context) {
    ...
    this._encoder = null;
    this._id = ++controlConnectionCounter;
    this._debouncer = new EventDebouncer(options.refreshSchemaDelay, this.log.bind(this), this._id);
  ...
  async _registerToConnectionEvents() {
    ...
    console.log(`[cc ${this._id}] Listener count on ${this.connection.endpointFriendlyName}: ${this.connection.listenerCount('nodeSchemaChange')}`);
  ...
  _nodeSchemaChangeHandler(event) {
    this.log('info', `[cc ${this._id}] Schema change`, event);
  ...
  handleSchemaChange(event, processNow) {
    ...
    if (event.isKeyspace) {
      if (event.schemaChangeType === schemaChangeTypes.dropped) {
        ...
        return this._scheduleObjectRefresh(handler, event.keyspace, null, processNow, event);
      }

      return this._scheduleKeyspaceRefresh(event.keyspace, processNow, event);
    }
   ...
   * @returns {Promise<void>}
   */
  _scheduleObjectRefresh(handler, keyspace, cqlObject, processNow, event = null) {
    return this._debouncer.eventReceived({ handler, keyspace, cqlObject, event}, processNow);
  }
   ...
   * @returns {Promise<void>}
   */
  _scheduleKeyspaceRefresh(keyspace, processNow, event = null) {
