...
class Metadata {
  ...
  async _refreshKeyspace(name) {
    ...
    try {
      ...
      if (!ksInfo) {
        ...
        this.log('info', util.format('Keyspace %s was not found on the server, removing it from metadata', name));
        delete this.keyspaces[name];
        return null;
      }
      // Tokens are lazily init on the keyspace, once a replica from that keyspace is retrieved.
      this.log('info', util.format('Setting metadata for keyspace %s', ksInfo.name));
  ...
  async refreshKeyspacesInternal(waitReconnect) {
    ...
    try {
      ...
      // Logs the wholesale replacement of the keyspaces map: if this list contains a keyspace that
      // a concurrent DROP just removed, it has been resurrected (see the metadata-refresh race).
      this.log('info',
        util.format('Replaced all keyspaces metadata, now tracking: [%s]', Object.keys(this.keyspaces).join(', ')));
