      ...
      return promiseUtils.toCallback(keyspaceEvents.mainEvent.handler(), function mainEventCallback(err) {
        for (let i = 0; i < keyspaceEvents.events.length; i++) {
          ...
          if (keyspaceEvents.events[i] !== keyspaceEvents.mainEvent) {
            ...
            console.log(util.format('Event for %s.%s is being dropped because a full refresh of the keyspace is scheduled', name, JSON.stringify(keyspaceEvents.events[i])));
          }
