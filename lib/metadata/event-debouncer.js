...
class EventDebouncer {
   ...
   */
  constructor(delay, logger, id) {
    this._delay = delay;
    this._logger = logger;
    this._queue = null;
    this._timeout = null;
    // Identifier used to correlate debouncer log lines with the owning ControlConnection
    this._id = id;
  }
...
function processQueue (q, id) {
  ...
  utils.each(Object.keys(q.keyspaces), function eachKeyspace(name, next) {
    ...
    console.log(`[deb ${id}] Current queue: ${JSON.stringify(q)}`);
    if (keyspaceEvents.mainEvent) {
      ...
      return promiseUtils.toCallback(keyspaceEvents.mainEvent.handler(), function mainEventCallback(err) {
        console.log(`[deb ${id}] Processed main event ${JSON.stringify(keyspaceEvents.mainEvent)}`);
        for (let i = 0; i < keyspaceEvents.events.length; i++) {
          // log if not main event so this event is getting dropped
          if (keyspaceEvents.events[i] !== keyspaceEvents.mainEvent) {
            // eslint-disable-next-line
            console.log(util.format('[deb %s] Event for %s.%s is being dropped because a full refresh of the keyspace is scheduled', id, name, JSON.stringify(keyspaceEvents.events[i])));
          }
